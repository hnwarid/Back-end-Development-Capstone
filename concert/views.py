    ...
    if request.method == 'POST': 
        ...
        try: 
            ...
            if user: 
                ...
                user = User.objects.create(username=username, password=make_pasword(password))
                login(request, user)
...
def concerts(request):
    if request.user.is_authenticated: 
        concerts_list = []
        concert_data = Concert.objects.all()
        ...
        for item in concert_data:
            try: 
                ...
                status  = '-'
            concert_data.append({'concert': item,'status': status})
